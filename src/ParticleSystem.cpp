...
void ThrusterForce:: updateForce(Particle* particle) {
	if (up)
		this->add(ofVec3f(0, .3, 0));
	else if (down)
		this->add(ofVec3f(0, -.3, 0));
	else if (left)
		this->add(ofVec3f(-0.3, 0, 0));
	else if (right)
		this->add(ofVec3f(0.3, 0, 0));
	else if (forward)
		this->add(ofVec3f(0, 0, .3));
	else if (back)
		this->add(ofVec3f(0, 0, -.3));
	
	particle->forces += velocity;

}

void ThrusterForce:: add(const ofVec3f move) {
	velocity += move;
}

void ThrusterForce::clear() {
